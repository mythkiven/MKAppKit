...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"Log";
    maxCount = [MKDebugTool shareInstance].maxLogsCount;
    if (maxCount == 0)
        maxCount = 50;
    ...
    [btnrefresh setTitleColor:[MKDebugTool shareInstance].mainColor forState:UIControlStateNormal];
...
- (void)loadLogs {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (arr.count > 0) {
            ...
            for (MKLogModel* model in arr) {
                NSString* date = [MKLogModel stringFormatFromDate:model.date];
                NSMutableAttributedString* att = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@:",date]];
                [att addAttribute:NSForegroundColorAttributeName value:[MKDebugTool shareInstance].mainColor range:NSMakeRange(0, att.string.length)];
...
- (NSArray* )logs {
    ...
    for (int i=0; i<numberOfLogs; i++) {
        ...
        if (msg != NULL) {
            MKLogModel* model = [MKLogModel messageFromASLMessage:msg];
