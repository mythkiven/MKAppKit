...
@interface JControlLoadingCircleLayer ()
@property (nonatomic, assign) CGFloat blockAngle;
@end

@implementation JControlLoadingCircleLayer
...
- (instancetype)initWithLayer:(id)layer {
    if (self = [super initWithLayer:layer]) {
        if ([layer isKindOfClass:[JControlLoadingCircleLayer class]]) {
        }
    }
    return self;
}
+ (id)layer {
    JControlLoadingCircleLayer *layer = [[JControlLoadingCircleLayer alloc] init];
    return layer;
}
